...
const Pokedex = () => {
  const { currentType, pokemonsByName, types, nameTrainer, namePokemon, handleSubmit, handleChangeType } = usePokedex();
        ...
        <blockquote data-arwes-global-palette="error">
          <p className="text-xl">
            <span className="text-red-700 font-bold">
              Welcome, {nameTrainer}! This is the place where your favorite Pokémon hangs out!
            </span>
          </p>
        </blockquote>
      </section>
      ...
      <section className=" mt-5 mb-10 flex flex-col sm:flex-row justify-center items-center">
        ...
        <form onSubmit={handleSubmit} className="flex">
          ...
          <input
            ...
            label="Type query"
            type="text"
            value={namePokemon} // Agregamos el valor del input
            onChange={(e) => handleChangeType(e)} // Cambiamos esto a handleChangeType
          />

          {/* Botón de búsqueda */}
          <button
            type="submit" // Cambiamos a type "submit" para que funcione el onSubmit
        ...
        {/* Selección por tipo */}
        <select onChange={handleChangeType} value={currentType} className=" capitalize mt-3 sm:mt-0 sm:ml-3">
          <option value="">By Type</option>
          {types.map((type) => (
            <option className="capitalize text-center" value={type.name} key={type.name}>
              {type.name}
            </option>
          ))}
        </select>
