...
const PokemonCard = ({ pokemonUrl }) => {
  ...
  return (
    <Link to={`/pokedex/${pokemon?.name}`}
      className={`text-center border-[${pokeLineaBorder[pokemon?.types[0]?.type?.name]}] relative rounded-bl-2xl rounded-br-2xl hover:[animation:scale-105]`}
    >
      <div className=" grid px-2 flex-col items-center justify-center rounded-2xl align-center">
        {/* Sección de la imagen */}
        <header
          className={`relative h-[80px] w-full mb-6 ${pokeLinearGradients[pokemon?.types[0]?.type?.name]
            }`}
        >
          <div className="absolute left-1/2 -translate-x-1/2 -bottom-10 h-[130px] aspect-square ">
