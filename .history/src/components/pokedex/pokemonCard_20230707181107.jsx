...
const PokemonCard = ({ pokemonUrl }) => {
  ...
  return (
    <article className="bg-white shadow-lg rounded-lg p-4">
  <section
  ...
  <section className="text-center mt-4">
    <h3 className="capitalize hover:uppercase text-3xl font-bold mb-2">
      {pokemon?.name}
    </h3>
    <span className="text-sm">Type: {formatTypesPokemon(pokemon?.types)}</span>
  </section>

  <section className="whitespace-normal mt-4">
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      {pokemon?.stats?.map((stat) => (
        <div
          className="bg-gray-100 p-2 border border-gray-300 rounded-lg"
          key={stat.stat.name}
        >
          {/* Tipo de ataque */}
          <h6 className="bg-red-600 text-white text-center capitalize">
            {stat.stat.name}
          </h6>
          {/* Fuerza del ataque */}
          <span className=" text-center bg-black text-white text-center">
            {stat.base_stat}
          </span>
        </div>
      ))}
